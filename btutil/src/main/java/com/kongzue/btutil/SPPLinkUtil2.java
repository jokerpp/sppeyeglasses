 ...
 */
public class SPPLinkUtil2 {
    
    public static boolean DEBUGMODE = true;                //是否打印日志
...
//    private String btName;
    ...
    public void initSppLink(Context c){
//        this.btName = btName;
        this.context = c;
...
//                            }
                ...
                if(DEBUGMODE){Log.d("ljp","蓝牙连接状态：连接");}
            }else if(BluetoothDevice.ACTION_ACL_DISCONNECTED.equals(action)){
                onBLEStatusChangeListener.onBleDisConnect();
                if(DEBUGMODE){Log.d("ljp","蓝牙连接状态：断开连接");}
                ...
                switch (state) {
                    case BluetoothAdapter.STATE_OFF:
                        if(DEBUGMODE)Log.d("aaa", "STATE_OFF 手机蓝牙关闭");
                        onBLEStatusChangeListener.onBleDisConnect();
                        onBLEStatusChangeListener.onBleClosed();
                        break;
                    case BluetoothAdapter.STATE_TURNING_OFF:
                        if(DEBUGMODE)Log.d("aaa", "STATE_TURNING_OFF 手机蓝牙正在关闭");
                        break;
                    case BluetoothAdapter.STATE_ON:
                        //通知蓝牙打开了
                        onBLEStatusChangeListener.onBleOpen();
                        if(DEBUGMODE)Log.d("aaa", "STATE_ON 手机蓝牙开启");
                        break;
                    case BluetoothAdapter.STATE_TURNING_ON:
                        if(DEBUGMODE) Log.d("aaa", "STATE_TURNING_ON 手机蓝牙正在开启");
                        break;
                }
            }
        }
        ...
        timer.schedule(new TimerTask() {
            public void run() {
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        //开始连接
                        if(DEBUGMODE)Log.d("ljp","进入SPPLinkUtil timer schedule.");
                        try {
...
//                                    Log.d("ljp","read:"+ ConvertData.bytesToHexString(read,true));
                                    if(onBtSocketResponseListener!=null) {
                                        ...
                                        if(DEBUGMODE)Log.d("ljp","收到消息回调接口为null");
                                    }
                                } catch (IOException e) {
                                    if(DEBUGMODE)Log.d("ljp","SPPLINKUtil loop read io exception:"+e.getMessage());
                                    break;
                                }
...
//                        }
    ...
    //TODO 增加直接连接
    public void connectImmediately(final String macAddress){
                                ...
                                while (true) { //死循环读取
                                    try {
                                        ...
                                        if(is == null || data == null){
                                            if(DEBUGMODE)Log.d("ljp","while循环is空");
                                            String msg = "";
                                            linkStatusChange(true, ERROR_SOCKET_ERROR, msg);
                                            return;
                                        }
...
//        isSending = false;
    ...
    public SPPLinkUtil2 setBtLinkReport(BtLinkReport btLinkReport) {
        this.btLinkReport = btLinkReport;
        return this;
    }
    
    public SPPLinkUtil2 setOnLinkStatusChangeListener(OnLinkStatusChangeListener onLinkStatusChangeListener) {
        this.onLinkStatusChangeListener = onLinkStatusChangeListener;
        return this;
    }
    
    public SPPLinkUtil2 setOnBtSocketResponseListener(OnBtSocketResponseListener onBtSocketResponseListener) {
        this.onBtSocketResponseListener = onBtSocketResponseListener;
        return this;
    }
    public SPPLinkUtil2 setOnDevFindListener(OnDevFindListener onLinkStatusChangeListener) {
        this.onDevFindLIstener = onLinkStatusChangeListener;
        return this;
    }
    public SPPLinkUtil2 setOnBleStatusChangeListener(OnBLEStatusChangeListener listener){
        this.onBLEStatusChangeListener = listener;
        return this;
    }
    public SPPLinkUtil2 setUUID(String UUID) {
        this.UUIDStr = UUID;
        return this;
    }
    
    public SPPLinkUtil2 setReadEndStr(String readEndStr) {
        this.readEndStr = readEndStr;
        return this;
    }
    
    public static void setBtPairingCode(String btPairingCode) {
        SPPLinkUtil2.btPairingCode = btPairingCode;
    }
...
//            btName = null;
    ...
    public static boolean setPin(Class<? extends BluetoothDevice> btClass, BluetoothDevice btDevice) throws Exception {
        ...
        try {
            ...
            if(DEBUGMODE)Log.e("returnValue222", "" + returnValue);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return true;
